    ...
    if (isset($_POST["daftar"])){
        ...
        if ($pengguna == "" or $nik == "" or $katasandi == "" or $konfirmasisandi == "") {
            ...
            if ($katasandi != $konfirmasisandi) {
                ...
                $admin = mysqli_query($conn, "SELECT * FROM admin WHERE NIK = '$nik'");
                if (mysqli_num_rows($admin) === 1) {
                    $err = "<b>$nik</b> sudah terdaftar !";
                }else {
                    $katasandi = password_hash($katasandi, PASSWORD_DEFAULT);
                    mysqli_query($conn, "INSERT INTO admin VALUES ('','$pengguna','$nik','$katasandi')");
                    $notif = "Admin berhasil ditambahkan !";
                }
            }
        }
    }
